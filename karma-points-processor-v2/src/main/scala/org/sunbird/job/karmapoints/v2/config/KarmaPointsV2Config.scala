...
class KarmaPointsV2Config(override val config: Config) extends BaseJobConfig(config, "program-karma-points-processor-v2") {
  ...
  // key only (RedisUtil.setPendingEnrolmentStatus) - every other Redis operation stays on cacheDbId.
  val pendingEnrolmentCacheDbId: Int = config.getInt("redis.database.pendingEnrolmentCache.id")
